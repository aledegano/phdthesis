...
int main(int argc, char** argv) {
	...
	if (argc != 2) {
		std::cerr << "One argument is required: 1 for CPU search, 2 for GPU search." << std::endl;
		return 1;
	}
	...
	if (searchType == 1)
		std::cout << "Executing CPU burnout." << std::endl;
	else
		std::cout << "Executing GPU burnout." << std::endl;
	...
	if (searchType == 2) {
		...
		start = std::chrono::system_clock::now();
		cudatree.search_in_the_box_linear(search_box, maxNN, arr_cuda_results, repeat);
		end = std::chrono::system_clock::now();

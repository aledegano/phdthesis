...
#include "CudaFKDTree.cuh"
#include "CUDAQueue.h"

__host__ __device__
bool intersects(unsigned int index, CudaFKDPoint minPoint, CudaFKDPoint maxPoint, int dim, CudaFKDPoint* theDimensions) {
	return (theDimensions[index][dim] <= maxPoint[dim] && theDimensions[index][dim] >= minPoint[dim]);
}

__host__ __device__
bool isInTheBox(unsigned int index, CudaFKDPoint minPoint, CudaFKDPoint maxPoint, CudaFKDPoint* theDimensions) {
	bool inTheBox = true;
...
__global__
void kernel_search_in_the_box_linear(unsigned int offset, const CudaFKDPoint* searchBox, unsigned int maxNN, CudaFKDPoint* theDimensions, const unsigned int theNumberOfPoints, unsigned int* result, unsigned int burnoutRepetitions) {
	for(unsigned int i=0; i<burnoutRepetitions; i++){
...
void CudaFKDTree::search_in_the_box_linear(CudaFKDPoint* searchBox, unsigned int maxNN, unsigned int* host_results, const unsigned int nStreams, const unsigned int threads_per_block, const unsigned int burnoutRepetitions) const {
	...
	for (unsigned int i = 0; i < nStreams; i++) {
		...
		kernel_search_in_the_box_linear<<<(streamSize + threads_per_block - 1) / threads_per_block, threads_per_block, 0, stream[i]>>>(offset, device_searchBox, maxNN, device_theDimensions, theNumberOfPoints, device_results, burnoutRepetitions);

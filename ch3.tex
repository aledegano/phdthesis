...
\Procedure{SearchKDTree}{searchVolume, nodeToSearch, depth}
  ...
  \State nearestNeighbors $\gets$ node[points] \Comment save all node points as NN
  \Else
...
Using the Standard Template Library (STL) pseudo-random uniform generator defined by the c++11 ISO standard we place $3 \times 10^{5}$ points in a cube of side 100 arbitrary units (a.u.). We build a volume tree with the generated points and for each of the points we perform a range search for a cube of side 4.0 a.u. centered on the point for which we want to search the NN and the results are recorded.\\
As comparison we perform again the nearest neighbors search through a method called \textit{naive} since it is the simplest possible way to perform such a search: for each search box every single point of the data set is checked if it lies inside the box it is recorded. It is also clear that the naive method is very inefficient having complexity $\mathcal{O}(n^{2})$ for this type of search. However it is a simple and reliable method to validate less intuitive algorithms and we used it extensively throughout this work.\\
The data set random generation is repeated for different number of data set points and different sizes of the search boxes, the results of the two methods are then compared for each generation and if they are always identical the kd-tree range search algorithm is declared valid.

\subsection{GPU porting}
The GPU architecture we described in Chapter \ref{ch:intro} requires some adaptations of the code to best perform on this device. We list here the most relevant:
\begin{itemize}
